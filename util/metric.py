...
class FalsePositive(metrics.Metric):
...
class FalseNegative(metrics.Metric):
    def __init__(self, name="false_negative", **kwargs):
        super(FalseNegative, self).__init__(name=name, **kwargs)
        self.false_negatives = self.add_weight(name="false_negative", initializer="zeros")
...
def equal_error_rate(y_true, y_pred, threshold=None):
    """
    y_true: tensor of shape (batch_size, )
    y_pred: tensor of shape (batch_size, embed_dim)
    """
    ...
    return false_positive, false_negative, threshold
