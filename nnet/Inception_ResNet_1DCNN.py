...
class Inception_ResNet:
    def __init__(self, length, num_channel, num_filters, problem_type='Regression',
                 output_nums=1, pooling='avg', dropout_rate=False, auxilliary_outputs=False, normalize=True):
    ...
    def MLP(self, x):
        ...
        if self.normalize:
            outputs = tf.keras.layers.Dense(self.output_nums, activation=None, name="latent")(x)
            outputs = tf.keras.layers.Lambda(lambda x: tf.math.l2_normalize(x, axis=1))(outputs)
        else:
            outputs = tf.keras.layers.Dense(self.output_nums, activation=None)(x)
    ...
    def Inception_ResNet_v2(self):
        ...
        # Stem
        x = Conv_1D_Block(x, 32, 3, strides=2, padding='valid')
        x = Conv_1D_Block(x, 32, 3, padding='valid')
        x = Conv_1D_Block(x, 64, 3)
        #
...
if __name__ == "__main__":
    model = Inception_ResNet(length=70, num_channel=5, num_filters=128, output_nums=512).Inception_ResNet_v2()
    model.summary()
